...
def spectHRplot(data, x_min=None, x_max=None):
    ...
    line_handler = LineHandler(fig, ax_ecg, data)

    def update_plot():
        """ Update the plot with current data and axis limits. """
        ecg_time, ecg_level = get_downsampled_ecg_data(data.ecg, x_min, x_max)
        plot_ecg_signal(ax_ecg, ecg_time, ecg_level)
...
def get_downsampled_ecg_data(ecg_data, x_min, x_max, max_points=1024):
    """ Get downsampled ECG data within the x_min and x_max range. """
    ecg_mask = (ecg_data.time >= x_min) & (ecg_data.time <= x_max)
    time, level = ecg_data.time[ecg_mask], ecg_data.level[ecg_mask]
    return downsample_data(time, level, max_points)


def downsample_data(time, level, max_points=1024):
    """ Downsample data to improve performance. """
    if len(time) > max_points:
        factor = max(1, len(time) // max_points)
        return time[::factor], level[::factor]
    return time, level
...
def plot_breathing_rate(ax, br_data, x_min, x_max):
    ...
    br_time, br_level = get_downsampled_breathing_data(br_data, x_min, x_max)
    ax.clear()
...
def get_downsampled_breathing_data(br_data, x_min, x_max, max_points=1024):
    """ Get downsampled breathing data within the x_min and x_max range. """
    br_mask = (br_data.time >= x_min) & (br_data.time <= x_max)
    return downsample_data(br_data.time[br_mask], br_data.level[br_mask], max_points)
