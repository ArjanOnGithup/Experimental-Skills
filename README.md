...
## Features
- It reads labstreaminglayers .XDF files. the ECG stream is detected if its label contains 'polar'. Generally for use with the  ![PolarGUI](https://github.com/markspan/PolarBand2lsl) application. Markers are ready from a seperate stream, and should follow the patterns '''start label''' and '''end label''' to mark an epoch.
